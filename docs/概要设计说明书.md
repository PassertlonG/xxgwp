...
| **payment** | `com.xxgwy.payment` | 微信支付统一下单、支付回调验签、支付幂等处理（与 order 模块协作） |
...
| Controller | 路径前缀 | 说明 |
|:---|:---|:---|
| `CategoryController` | `/api/categories` | 分类树查询/管理 |
...
- **orders → return_orders**：一个订单可有多条退款记录（一期整单退款）
...
| payment 表（支付记录） | (wx_transaction_id, order_no) | UNIQUE COMPOSITE INDEX | 支付幂等：防重复回调 |
...
| ADMIN | `/api/admin/**`、`/api/categories` (POST/PUT/DELETE)、`/api/products` (POST/PUT/PATCH) |
...
│    current = Redis.GET(sku:{skuId}:stock)        │
│    if current < quantity:                         │
│      → 事务回滚 → 返回 HTTP 409 库存不足          │
│                                                   │
│  for each (skuId, quantity) in orderItems:       │
│    Redis.DECR(sku:{skuId}:stock, quantity)        │
│                                                 │
...
**为什么用 Redis DECR 而非直接 PostgreSQL 行级锁**：

- Redis 单线程，DECR 原子，无锁等待
- 高并发下 MySQL/PostgreSQL 行级锁可能造成死锁或等待超时
- Redis 是库存的"前置防线"，扣减成功后再写 PostgreSQL，保证最终一致性

**兜底方案**：PostgreSQL 中 `product_skus.stock` 字段保留，Redis 缓存与 DB 定期同步（或使用 `SELECT ... FOR UPDATE` 作为二次校验）。
...
```kotlin
// 优惠公式: discount = ceil(quantity / 2) * price * (1 - discountRate)
// 其中 ceil(quantity/2) 为可享受折扣的件数（第2、4、6...件）
fun calcDiscount(price: BigDecimal, quantity: Int, discountRate: BigDecimal): BigDecimal {
    val discountCount = (quantity + 1) / 2  // 第2、4、6...件为 floor(quantity/2), 
                                             // 这里取 ceil 是考虑第2件打折，
                                             // 实际应为 quantity / 2 向下取整
    val actualDiscountCount = quantity / 2
    return price.multiply(BigDecimal(actualDiscountCount))
        .multiply(BigDecimal.ONE.subtract(discountRate))
}
```

> 注：以上为伪代码示例。实际实现需根据甲方确认的折扣件数规则（仅第 2 件打折、还是第 2/4/6... 件都打折）精确计算。
