...
| UC103 | 厂家/管理员账号登入               | 厂家、管理员      | 在 H5 后台通过账号+密码+验证码登录系统                                                    |
...
前置条件 ： 使用 "湘西果王油微信小程序 软件" 时

交互动作： 
1. **用户**点击 使用微信 登入 
2. **系统**验证 code , 如果 微信接口服务 报错
   1. 提示 用户 微信 code 错误，重新进入 登入页面
...
前置条件 ： 使用 "湘西果王油后台管理h5 软件" 时

交互动作：
1. **用户** 输入账号、密码和验证码（如果有
2. **湘西果王油后台管理h5 软件** 验证用户输入账号和密码的合法性，如不合法
   1. **湘西果王油后台管理h5 软件** 提示并要求重新输入账号和密码
3. **系统** 验证用户账号、密码和验证码（如果有）的正确性
4. 如正确则登入成功。否则
   1. 提示用户并要求重新输入账号和密码，转到 2
   2. 如果输入账号和密码登入不超过次数超过3次，在后续的登入流程中，加入验证码验证流程，直到登入成功
   3. 如果用户要求提示密码，则提示请联系管理员
...
| 编号    | 用例名称     | 角色  | 用例描述                                                                |
|:------|:---------|:----|:--------------------------------------------------------------------|
| UC201 | 按分类浏览商品  | 消费者 | 在首页按分类浏览商品，展示树形分类列表，点击进入该分类下的商品列表                                   |
| UC202 | 查看商品详情   | 消费者 | 查看商品轮播图、富文本描述、所有规格（名称/价格/库存），仅展示已上架商品                               |
| UC203 | 关键字搜索商品  | 消费者 | 按关键字搜索商品，后端保留 `/api/products?keyword=` 接口，前端暂不接入，留至第四期上线              |
| UC204 | 商品管理     | 厂家  | 在后台对商品进行增删改查、上下架管理，上架后消费者端可见，下架后数据保留                                |
| UC205 | 分类管理     | 厂家  | 对分类进行增删改查管理（树形结构，无限层级），删除分类前校验其下无商品                                |
...
| 角色               | 可访问接口                                                                                                                                                   |
|:-----------------|:--------------------------------------------------------------------------------------------------------------------------------------------------------|
| 匿名（未登录）          | `/api/categories`（GET）、`/api/products`（GET）、`/api/auth/wx-login`、`/api/auth/login`、`/api/orders/wx-notify`                                                                |
