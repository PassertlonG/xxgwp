...
| 编号 | 角色 | 用户故事 | 验收标准 |
|:---|:---|:---|:---|
| UC101 | 消费者、厂家、管理员 | 通过微信授权登录系统 | 调用微信 code2session 接口，校验角色存在且未被禁用后生成 JWT 返回；消费者和厂家首次授权时自动注册用户记录 |
| UC102 | 消费者 | 通过微信授权注册为新用户 | 首次授权时系统自动创建用户记录（角色=CUSTOMER），生成 JWT 返回；已注册用户再次授权直接登录 |
| UC103 | 管理员 | 添加或删除厂家用户 | 管理员在后台创建/删除厂家账号（角色=MANUFACTURER）；删除操作为软删除；不能删除自己 |
| UC104 | 消费者 | 查询和编辑个人信息（昵称、头像、手机号） | `GET /api/user/me` 返回当前用户信息；`PUT /api/user/me` 支持更新昵称和头像，手机号仅可从微信授权获取 |
| UC105 | 消费者 | 新增、编辑、删除收货地址，设置默认地址 | 地址 CRUD 接口完整可用，默认地址唯一约束（设置新默认时旧默认自动取消），最多 20 个地址 |
| UC106 | 厂家 | 查询所有用户的个人信息 | `GET /api/admin/users` 返回分页用户列表，支持按手机号搜索 |
### 3.1.2 业务流程
```plantuml
@startuml
...
usecase "用户登入" as UC101
usecase "用户注册" as UC102
usecase "添加/删除 厂家用户" as UC103
usecase "用户 查询和编辑 个人信息（名称、头衔、手机号）" as UC104
usecase "新增、编辑、删除收货地址，设置默认地址" as UC105
usecase "用户 查询 所有人的 个人信息（名称、头衔、手机号）" as UC106 
...
- **UC101**：任一角色调用 `/api/auth/wx-login` → 后端 code2session → 校验角色存在且未被禁用 → 返回 JWT；角色不存在或已禁用 → 返回 403
- **UC102**：新用户首次微信授权 → 系统自动插入 users 记录（角色=CUSTOMER）→ 返回 JWT；已注册用户再次授权 → 直接登录
- **UC103**：管理员访问后台创建厂家账号 → users 表新增 MANUFACTURER 角色记录；删除厂家 → 软删除 `is_deleted=true`；删除不存在的用户 → 返回 404；删除自己 → 返回 400
- **UC104**：`PUT /api/user/me` 更新后响应体返回最新数据；手机号字段不可通过此接口修改
- **UC105**：新建地址 → 列表中出现新地址；删除地址 → 列表中消失（软删除）；设置默认 → `GET /api/addresses` 返回的列表中仅一个 `is_default=true`
- **UC106**：厂家登录后访问 `/api/admin/users` → 返回所有人分页列表（不含已删除用户）；普通消费者访问 → 返回 403
...
| PR-03 | 消费者 | 按关键字搜索商品（第四期实现） | 后端保留 `/api/products?keyword=` 接口，前端暂不接入，留至第四期上线 |
| PR-04 | 管理员 | 在后台对商品进行增删改查、上下架管理 | 后台商品 CRUD 完整；上架后商品在消费者端可见,下架后不可见但数据保留                   |
| PR-05 | 管理员 | 对分类进行增删改查管理（树形结构） | 后台分类 CRUD 支持无限层级；删除分类前校验其下无商品；分类树结构通过 `parent_id` 自引用实现 |
...
- **PR-03**：`GET /api/products?keyword=xxx` 接口存在且返回匹配结果；前端不调用此接口，留至第四期
- **PR-04**：上架操作 → `PATCH /api/products/{id}/enable` → 状态变为 ENABLED → 消费者端可见
- **PR-05**：删除有商品的分类 → 返回 400 错误码 `1002`，提示"该分类下存在商品，无法删除"
...
| 编号 | 角色 | 用户故事 | 验收标准 |
|:---|:---|:---|:---|
| OR-01 | 消费者 | 在商品详情页选择规格和数量后点击购买，进入订单确认页查看商品信息、总价、选择收货地址 | 确认页展示商品名、规格名、数量、单价、总价、收货地址；地址可切换和新增 |
| OR-02 | 消费者 | 在确认页修改购买数量，确认总价后点击支付 | 数量可调，总价实时计算（单价 × 数量）；一期无优惠，`pay_amount = total_amount` |
| OR-03 | 消费者 | 点击支付按钮后，系统创建订单并唤起微信支付，支付成功后跳转订单详情 | 点击后创建订单（状态=待支付），返回 JSAPI 支付参数，前端唤起支付弹窗，支付成功回调后状态变更为已支付 |
| OR-04 | 消费者 | 在订单列表中看到所有订单状态，并可查看详情 | 订单列表分页，支持按状态筛选；详情页展示商品明细、收货地址快照、金额拆解（商品小计 + 实付款 = 总价，一期无折扣时二者相等）、当前状态 |
...
- **OR-02**：数量调整为 0 或负数时前端拦截，后端返回 400；总价 = 单价 × 数量
...
Payload: { "sub": "<user_id>", "role": "CUSTOMER|MANUFACTURER|ADMIN", "iat": ..., "exp": ... }
...
| ADMIN | `/api/admin/**`（全部管理端接口，含厂家用户管理） |
...
| POST | `/api/admin/users` | 创建厂家用户 | ADMIN | `{ openid, phone?, nickname? }` | `UserDTO` |
...
| GET | `/api/products` | 商品列表（分页，按分类筛选） | 公开 |
...
| v2.1 | 2026-05-01 | opencode | 甲方批阅修正：删除 PR-03 排序筛选；PR-04 关键字搜索后端保留接口、前端留至第四期；OR-02 第二件打折延至第四期；OR-04 金额拆解释义；相关数据字典、验收标准同步更新 |
